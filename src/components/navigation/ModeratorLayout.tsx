...
import { Package, Plus, Briefcase, MessageCircle, Users, Star, User } from 'lucide-react';
...
type ModeratorView =
  ...
  | 'create' | 'messages' | 'moderation' | 'trust' | 'profile';

export default function ModeratorLayout() {
  const { profile } = useAuth();
  const [view, setView] = useState<ModeratorView>('home');
  ...
  useEffect(() => {
    ...
    // Rafraîchir le badge après modération
  ...
  return (
    <>
      <NavBar
        items={navItems}
        currentView={view}
        onNavigate={(v) => setView(v as ModeratorView)}
        accentColor="bg-purple-600"
      />

      <div className="bg-white rounded-xl shadow-sm p-6">
        {view === 'home' && (
          <HomePanel onNavigate={(v) => setView(v as ModeratorView)} />
        )}
        {view === 'announcements' && (
          <AnnouncementList
